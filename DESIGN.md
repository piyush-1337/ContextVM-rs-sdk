...
|---------------|--------|-------------|--------|--------|
| `core/constants.ts` | 87 | `core/constants.rs` | Port from existing Rust | ✅ exists |
| `core/interfaces.ts` | 61 | `core/types.rs` | Port from existing Rust | ✅ exists |
| `core/encryption.ts` | 64 | `encryption/mod.rs` | Port from existing Rust | ✅ exists |
| `core/utils/serializers.ts` | ~60 | `core/serializers.rs` | New | ❌ |
| `core/utils/utils.ts` | ~30 | `core/validation.rs` | New | ❌ |
| `relay/simple-relay-pool.ts` | ~100 | `relay/mod.rs` | Port from existing Rust | ✅ exists |
| `signer/private-key-signer.ts` | ~50 | `signer/mod.rs` | Port from existing Rust | ✅ exists |
| `transport/base-nostr-transport.ts` | 355 | `transport/base.rs` | New | ❌ |
| `transport/nostr-client-transport.ts` | 411 | `transport/client.rs` | Rewrite from existing | 🔶 partial |
| `transport/nostr-server-transport.ts` | 944 | `transport/server.rs` | Rewrite from existing | 🔶 partial |
| `gateway/index.ts` | 151 | `gateway/mod.rs` | New | ❌ |
| `proxy/index.ts` | 96 | `proxy/mod.rs` | New | ❌ |

**TS SDK total:** ~2,169 LOC (non-test)  
**Existing Rust:** ~782 LOC  
**Estimated new Rust:** ~2,000-2,500 LOC
...
|-------|--------|-------------|
| Phase 1: Core | ~4h | None |
| Phase 2: Transport | ~8h | Phase 1 |
| Phase 3: Gateway & Proxy | ~3h | Phase 2 |
| Phase 4: Discovery | ~3h | Phase 2 |
| Phase 5: Examples & Docs | ~2h | Phase 3+4 |
| **Total** | **~20h** | |
